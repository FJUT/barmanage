...
router.get("/getTopRankUsers", (req, res, next) => {
  ...
  co(function*() {
    ...
    if (type == 1 || type == 3) {
      _sql_ =
        "SELECT tmp.UserId, tmp.avatar, tmp.name, tmp.consume, tmp.gender, tmp.bpcount FROM (SELECT u.id UserId, u.avatar, u.name name, u.gender, SUM(o.amount) consume, count(o.amount) bpcount FROM Users u, Orders o WHERE o.status = 1 AND o.UserId = u.id GROUP BY u.id) tmp ORDER BY tmp.consume DESC LIMIT 100";
    } else if (type == 2) {
      _sql_ =
        "SELECT tmp.UserId, tmp.avatar, tmp.name, tmp.consume, tmp.gender, tmp.bpcount FROM (SELECT u.id UserId, u.avatar, u.name name, u.gender, SUM(o.amount) consume, count(o.amount) bpcount FROM Users u, Orders o WHERE o.status = 1 AND o.UserId = u.id GROUP BY u.id) tmp ORDER BY tmp.bpcount DESC LIMIT 100";
    } else {
    ...
    if (_query_result && _query_result[0]) {
      ...
      if (_mapRes) res.json({ iRet: 0, data: _mapRes });
      else res.json({ iRet: -1, msg: "[3]计算结果出错" });
    } else res.json({ iRet: 0, data: {}, msg: "数据库查无结果" });
  }).catch(function(err) {
...
router.get("/getLevel", (req, res, next) => {
  ...
  co(function*() {
    let consumerSum = yield Order.sum("amount", {
...
router.get("/landbar", (req, res, next) => {
  ...
  co(function*() {
    ...
    if (bar && user) {
      ...
      //没有就直接插入
      if (_record.length < 1) {
        let landInfo = yield models.LandInfo.create({
          userid: userid,
          barid: barid
        });
        if (landInfo) res.json({ iRet: 0, msg: "插入记录成功" });
        else res.json({ iRet: -1, msg: "插入landinfo失败" });
      } else {
...
router.get("/getPageMessages", (req, res, next) => {
  ...
  DataApi.getPageMessages(id, limit, offset).then(data => {
    res.send(data);
  });
});
...
const createPayMiddware = (req, res, next) => {
  ...
  chklist &&
    chklist["white"] &&
    chklist["white"].forEach((chkobj, i, arr) => {
...
// 接收微信回调中间件
const responseWeixinNotifyMiddware = notifyMiddleware
  .getNotify()
  .done((message, req, res, next) => {
    ...
    co(function*() {
      ...
      //更新用户exp和score
      var updateExpAndScoreResult = yield User.findOne({
        where: { id: userId }
      }).then(function(user) {
