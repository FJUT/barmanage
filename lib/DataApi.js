...
const DataApi = {
  ...
  getPageMessages(barId, limit, offset) {
    ...
    return co(function*() {
      //计算个数
      let _sql_count = `SELECT COUNT(*) count FROM Messages WHERE BarId = ${barId} AND createdAt > DATE_SUB(NOW(), INTERVAL 24 HOUR)`
      let _count_result = yield sequelize.query(_sql_count)
      let _count = _count_result[0] && _count_result[0][0] ? _count_result[0][0]['count'] : 0

      // 找用户
      let _sql_users = `SELECT u.avatar UserAvatar, u.name UserName, u.gender, u.exp, m.msgText, m.msgImage, m.msgVideo, m.msgType, m.createdAt, m.updatedAt \
