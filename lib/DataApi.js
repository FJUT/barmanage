...
const DataApi = {
  ...
  getPageMessages(barId, limit, offset) {
    let me = this
    return co(function*() {
      let _sql_count = `SELECT COUNT(*) count FROM Messages WHERE BarId = ${barId} AND createdAt > DATE_SUB(NOW(), INTERVAL 3 HOUR)`
      ...
      let _sql_users = `SELECT u.avatar UserAvatar, u.name UserName, u.gender, u.exp, m.msgText, m.msgImage, m.msgVideo, m.msgType \
        ...
        AND m.createdAt > DATE_SUB(NOW(), INTERVAL 3 HOUR)`
